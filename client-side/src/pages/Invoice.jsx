...
const Invoice = () => {
  ...
  return (
    <div className="font-['Poppins',_sans-serif] bg-[#f8f9fa] text-[#333] p-5 w-full max-w-full">
      <div className="invoice-container max-w-[800px] w-full my-[30px] mx-auto bg-[#fff] border-[3px] border-solid border-[#FFBE86] rounded-[8px] shadow-md p-[40px] max-xs:p-[13px]">

        {/* Header */}
        <div className="invoice-header flex flex-wrap justify-between items-center border-b-[1px_solid_#eee] pb-[20px] mb-[20px]">
        ...
        <div className="invoice-details grid grid-cols-1 md:grid-cols-2 gap-6 text-sm mb-[25px]">
          ...
          <div>
            ...
            <p className="break-words">#INV-{data?._id}</p>
          </div>

          <div className="md:text-right">
            <p className="font-semibold text-gray-600">DATE:</p>
            <p className="break-words">{data?.createdAt}</p>
          </div>

          <div>
            <p className="font-semibold text-gray-600">BILLED TO:</p>
            <p className="font-medium">{data?.customer.name}</p>
            <p className="break-words">{data?.deliveryAddress}</p>
            <p className="break-words">{data?.customer.email}</p>
          </div>

          <div className="md:text-right">
            <p className="font-semibold">ORDER NO:</p>
            <p className="break-words">#{data?._id}</p>
          </div>
        </div>
        ...
        <div className="item-table mb-[25px] overflow-x-auto w-full">
          <table className="w-full min-w-[600px]">
            <thead>
              <tr>
                <th className="w-1/2 text-left max-xs:text-[12px]">ITEM DESCRIPTION</th>
            ...
            <tbody>
              {data?.subOrders?.flatMap((sub) => sub.items)?.map((item) => (
                <tr key={item?.food?._id} className="border-b border-[#eee]">
                  <td className="break-words">{item?.food?.name}</td>
                  <td className="text-center">{item?.quantity}</td>
                  <td className="text-right">$ {item?.food?.price?.toFixed(2)}</td>
        ...
        <div className="summary-table flex justify-end mb-[25px]">
          <table className="w-full md:w-1/2">
            <tbody>
              <tr>
                <td>SUBTOTAL</td>
                <td className="text-right">$ {data?.totalPrice?.toFixed(2)}</td>
              </tr>
              <tr>
                <td>TAX (5%)</td>
                <td className="text-right">$ {(data?.totalPrice * 0.05)?.toFixed(2)}</td>
              </tr>
              <tr className="text-lg">
                <td>GRAND TOTAL</td>
                <td className="font-bold text-right">$ {(data?.totalPrice * 1.05)?.toFixed(2)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="no-print flex flex-wrap justify-center gap-4 mt-8">
          <button
            ...
            className="bg-[#f0f0f0] text-[#555] p-[12px_25px] rounded-[8px] font-[600]"
          >
          ...
          <button
            onClick={downloadPDF}
            className="bg-[#F97316] text-white p-[12px_25px] rounded-[8px] font-[600]"
          >
