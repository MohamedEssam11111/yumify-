...
const Chatbot = () => {
  ...
  const Config = isMd
    ? {
        ...
        chefHatSize: 0.55,
        chefHatRotate: 0,
        chefHatPosition: { x: -60, y: -8 },

        leftHandSize: 1,
        leftHandRotate: 0,
        leftHandPosition: { x: 0, y: -10 },

        rightHandSize: 1,
        rightHandRotate: 0,
        rightHandPosition: { x: 0, y: -15 },

        openEyesSize: 1,
        openEyesRotate: 0,
        openEyesPosition: { x: -39, y: -15 },

        closeEyesSize: 1,
        closeEyesRotate: 0,
        closeEyesPosition: { x: -39, y: -15 },
      }
    : {
        scale: 0.8,
        ...
        chefHatSize: 0.45,
        chefHatRotate: 0,
        chefHatPosition: { x: -55, y: 8 },

        leftHandSize: 0.9,
        leftHandRotate: 0,
        leftHandPosition: { x: 6, y: -9 },

        rightHandSize: 0.9,
        rightHandRotate: 0,
        rightHandPosition: { x: -6, y: -12 },

        openEyesSize: 0.9,
        openEyesRotate: 0,
        openEyesPosition: { x: -35, y: -11 },
  ...
  return (
    <>
      ...
      <Ymym
        ...
        bodyPosition={Config.bodyPosition}
        chefHatSize={Config.chefHatSize}
        chefHatRotate={Config.chefHatRotate}
        chefHatPosition={Config.chefHatPosition}
        leftHandSize={Config.leftHandSize}
        leftHandRotate={Config.leftHandRotate}
        leftHandPosition={Config.leftHandPosition}
        rightHandSize={Config.rightHandSize}
        rightHandRotate={Config.rightHandRotate}
        rightHandPosition={Config.rightHandPosition}
        openEyesSize={Config.openEyesSize}
        openEyesRotate={Config.openEyesRotate}
        openEyesPosition={Config.openEyesPosition}
        closeEyesSize={Config.closeEyesSize}
        closeEyesRotate={Config.closeEyesRotate}
        closeEyesPosition={Config.closeEyesPosition}
        onClick={toggleChat}
      >
        <button
          onClick={toggleChat}
          className={`fixed bottom-4 right-[64px] md:bottom-8 md:right-[70px]
            z-50
            w-[60px] h-[60px] md:w-[76px] md:h-[76px] rounded-full 
            bg-cover
            bg-center
            bg-no-repeat
          ...
          style={{
            backgroundImage: `url(${messageCircleIcon})`,
            backgroundSize: "260%",
          }}
          aria-label="Open chatbot"
        >
          {messages.length > 0 && !isOpen && (
            <span className="absolute -top-[8px] -right-[8px] bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-medium">
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              <img
                src={YmymIcon}
                className="absolute top-0 left-0 w-13 h-12 rounded object-cover "
              />

              <h3 className="font-semibold text-lg ml-7">Ymym</h3>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={toggleTheme}
              ...
              >
                ...
                >
                  {message.isLoading ? (
                    ...
                    <div
                      className={`text-sm break-words markdown-content ${message.role === "user" ? "text-white" : darkMode ? "text-gray-100" : "text-gray-800"}`}
                    >
                      <ReactMarkdown
                        components={{
                          p: ({ children }) => (
                          ...
                          ul: ({ children }) => (
                            <ul
                              className="list-disc list-inside mb-2 space-y-1 ml-1"
                              style={{
                                marginLeft: "0.5rem",
                                marginBottom: "0.5rem",
                              }}
                          ...
                          ol: ({ children }) => (
                            <ol
                              className="list-decimal list-inside mb-2 space-y-1 ml-1"
                              style={{
                                marginLeft: "0.5rem",
                                marginBottom: "0.5rem",
                              }}
